...
export async function GET() {
  try {
    ...
    const { data: repos } = await db.from("creators")
      .select("handle, repo_full_name, repo_stars, agent_label, avatar_url, total_earned_usdc, owner_account_id, is_repo")
      .eq("is_repo", true);
    ...
    const byDev = new Map<string, { owner: string; avatar: string | null; repos: { handle: string; repoFullName: string; repoStars: number; agentLabel: string; earned: number }[] }>();
    for (const r of repos ?? []) {
      ...
      byDev.get(owner)!.repos.push({
        ...
        agentLabel: r.agent_label, earned: Number(r.total_earned_usdc ?? 0),
      });
    }
